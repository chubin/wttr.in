...
func NewCache(config *Config) (*Cache, error) {
	db, err := godb.Open(sqlite.Adapter, config.IPCacheDB)
	if err != nil {
		return nil, err
	}
...
func (c *Cache) cacheFile(addr string) string {
	return path.Join(c.config.IPCache, addr)
}
...
func (c *Cache) Response(r *http.Request) *types.Cadre {
	var (
		respERR = &types.Cadre{Body: []byte("ERR")}
		respOK  = &types.Cadre{Body: []byte("OK")}
	)
	...
	if r.URL.Path == "/:geo-ip-get" {
		...
		return &types.Cadre{Body: []byte(result.String())}
	}
